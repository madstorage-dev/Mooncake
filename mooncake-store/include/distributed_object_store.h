...
enum class OperationType : uint8_t { PUT, GET, REMOVE, REPLICATE };
...
class DistributedObjectStore {
   public:
    ...
    TaskID put(ObjectKey key, const std::vector<Slice> &slices,
               ReplicateConfig config);

    TaskID get(ObjectKey key, std::vector<Slice> &slices, Version min_version,
               size_t offset);

    TaskID remove(ObjectKey key, Version version = -1);

    TaskID replicate(ObjectKey key, ReplicateConfig new_config,
   ...
   private:
    ...
    void handlePutCompletion(std::shared_ptr<TaskContext> context,
                             const std::vector<TransferStatusEnum> &status);

   private:
    // define atomic taskid
    std::atomic<TaskID> task_id_;
    std::unordered_map<TaskID, std::shared_ptr<TaskContext>> task_contexts_;
