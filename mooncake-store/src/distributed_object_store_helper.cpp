...
int PyDistributedObjectStore::register_buffer(const std::string &segment_name, uint64_t start_addr, size_t size)
{
...
uint64_t PyDistributedObjectStore::put_object(const std::string &key, const std::string &data)
{
...
std::string PyDistributedObjectStore::get_object(const std::string &key, size_t data_size, uint64_t min_version, uint64_t offset)
{
    std::lock_guard<std::mutex> guard(mutex_);
    Slice slice;
    slice.ptr = start_addr_;
