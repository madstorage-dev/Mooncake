...
void RdmaTransferAgent::init() {
    ...
    auto nic_priority_matrix_content =
        loadNicPriorityMatrix(nic_priority_matrix);
    ...
    transfer_engine_->init(local_server_name.c_str(), connectable_name.c_str(),
                           12345);
    rdma_engine_ = static_cast<RdmaTransport *>(
        transfer_engine_->installOrGetTransport("rdma", args));
...
void *RdmaTransferAgent::allocateLocalMemory(size_t buffer_size) {
    ...
    int rc = transfer_engine_->registerLocalMemory(address, buffer_size,
                                                   "cpu:" + std::to_string(0));
    LOG_ASSERT(!rc);
    return address;
}

bool RdmaTransferAgent::doWrite(
...
bool RdmaTransferAgent::doReplica(
    const std::vector<TransferRequest> &transfer_tasks,
    std::vector<TransferStatusEnum> &transfer_status) {
    return doTransfers(transfer_tasks, transfer_status);
}

bool RdmaTransferAgent::doTransfers(
    const std::vector<TransferRequest> &transfer_tasks,
    std::vector<TransferStatusEnum> &transfer_status) {
    ...
    for (size_t task_id = 0; task_id < transfer_tasks.size(); ++task_id) {
        ...
        while (!completed && !failed) {
            int ret =
                rdma_engine_->getTransferStatus(batch_id, task_id, status);
            LOG_ASSERT(!ret);
...
// 异步提交传输请求
BatchID RdmaTransferAgent::submitTransfersAsync(
    const std::vector<TransferRequest> &transfer_tasks) {
    auto batch_id = rdma_engine_->allocateBatchID(transfer_tasks.size());
    int ret = rdma_engine_->submitTransfer(batch_id, transfer_tasks);
    if (ret != 0) {
...
void RdmaTransferAgent::monitorTransferStatus(
    BatchID batch_id, size_t task_count,
    std::vector<TransferStatusEnum> &transfer_status) {
    // std::vector<TransferStatusEnum> transfer_status(task_count,
        ...
        for (size_t task_id = 0; task_id < task_count; ++task_id) {
            if (transfer_status[task_id] == TransferStatusEnum::PENDING) {
                ...
                if (ret == 0) {
                    ...
                    LOG(ERROR) << "the task_id: " << task_id
                               << " , status: " << status.s;
                } else {
                    LOG(ERROR) << "Failed to get transfer status, batch_id: "
                               << batch_id << ", task_id: " << task_id;
                }
            }
        }
        std::this_thread::sleep_for(
            std::chrono::milliseconds(10));  // 避免频繁轮询
    }
