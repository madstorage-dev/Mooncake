...
DistributedObjectStore::DistributedObjectStore()
    : replica_allocator_(
          std::stoi(ConfigManager::getInstance().get("shard_size", "65536"))),
      allocation_strategy_(nullptr),
      max_trynum_(
          std::stoi(ConfigManager::getInstance().get("max_trynum", "10"))) {
...
uint64_t DistributedObjectStore::registerBuffer(SegmentId segment_id,
                                                size_t base, size_t size) {
    return replica_allocator_.registerBuffer(segment_id, base, size);
}

void DistributedObjectStore::unregisterBuffer(SegmentId segment_id,
                                              uint64_t index) {
    std::vector<std::shared_ptr<BufHandle>> need_reassign_buffers =
        replica_allocator_.unregister(segment_id, index);
...
void DistributedObjectStore::updateReplicaStatus(
    ...
    const std::vector<TransferStatusEnum> &status, const std::string &key,
    const Version version, ReplicaInfo &replica_info) {
    ...
    for (size_t i = 0; i < requests.size(); ++i) {
        ...
        if (status[i] != TransferStatusEnum::COMPLETED) {
            ...
            if (failed_index.find(handle_index) == failed_index.end()) {
                replica_info.handles[handle_index]->status =
                    BufStatus::COMPLETE;
            }
        }
    ...
    if (ifCompleted) {
        replica_allocator_.updateStatus(key, ReplicaStatus::COMPLETE,
                                        replica_info.replica_id, version);
        LOG(INFO) << "Key " << key << ", Replica " << replica_info.replica_id
                  << " completed";
    }
}

TaskID DistributedObjectStore::put(ObjectKey key,
    ...
    for (int index = 0; index < replica_num; ++index) {
        if (first_add) {
            version = replica_allocator_.addOneReplica(
                key, replica_infos[index], DEFAULT_VALUE, total_size,
                allocation_strategy_);
        } else {
            version = replica_allocator_.addOneReplica(
                key, replica_infos[index], version, DEFAULT_VALUE,
                allocation_strategy_);
        }
        ...
        generateWriteTransferRequests(replica_infos[index], slices,
                                      all_requests[index]);
    }
    ...
    for (const auto &requests : all_requests) {
        combined_requests.insert(combined_requests.end(), requests.begin(),
                                 requests.end());
    }
    ...
    LOG(INFO) << "Put object submitted asynchronously, key: " << key
              << ", version: " << version << ", replica_num: " << replica_num;
...
void DistributedObjectStore::handlePutCompletion(
    ...
    if (succeed_num == 0) {
        ...
        for (int index = 0; index < context->replica_num; ++index) {
            ReplicaInfo ret;
            replica_allocator_.removeOneReplica(context->key, ret,
                                                context->version);
        }
        LOG(WARNING) << "No replica succeeded when putting, key: "
                     << context->key
                     << ", replica_num: " << context->replica_num;
    } else {
        LOG(INFO) << "Put object completed asynchronously, key: "
                  << context->key << ", succeed num: " << succeed_num
                  << ", needed replica num: " << context->replica_num;
    }
}
TaskID DistributedObjectStore::get(ObjectKey key, std::vector<Slice> &slices,
                                   Version min_version, size_t offset) {
    ...
    ver = replica_allocator_.getOneReplica(key, replica_info, min_version,
                                           allocation_strategy_);
    if (ver > ERRNO_BASE) {
    ...
    while (!success && trynum < max_trynum_) {
        ...
        ++trynum;
        LOG(WARNING) << "Retrying, trynum: " << trynum << ", key: " << key;
    }

    if (trynum == max_trynum_) {
        LOG(ERROR) << "Read data failed after max tries: " << max_trynum_
                   << ", key: " << key;
        return getError(ERRNO::INVALID_READ);
    }
...
TaskID DistributedObjectStore::remove(ObjectKey key, Version version) {
    ...
    while (replica_allocator_.removeOneReplica(key, info, version) <=
           ERRNO_BASE) {
...
TaskID DistributedObjectStore::replicate(ObjectKey key,
    ...
    size_t existed_replica_number =
        replica_allocator_.getReplicaRealNumber(key, latest_version);
    if (existed_replica_number == 0) {
        LOG(ERROR) << "Failed to get existed replica number, no complete "
                      "replica in this version, key: "
    ...
    if (new_config.replica_num > existed_replica_number) {
        size_t add_replica_num =
            new_config.replica_num - existed_replica_number;
        ...
        for (size_t i = 0; i < add_replica_num; ++i) {
            ...
            Version existed_version = replica_allocator_.getOneReplica(
                key, existed_replica_info, latest_version,
                allocation_strategy_);
            if (existed_version > ERRNO_BASE) {
                LOG(ERROR) << "Failed to get existed replica in replicate "
                              "operation, key: "
                           << key << ", needed version: " << latest_version;
                return existed_version;
            }

            Version add_version = replica_allocator_.addOneReplica(
                key, new_replica_info, existed_version, DEFAULT_VALUE,
                allocation_strategy_);
            if (add_version > ERRNO_BASE) {
                LOG(ERROR)
                    << "Failed to add replica in replicate operation, key: "
                    << key << ", needed version: " << latest_version;
                return add_version;
            }

            assert(existed_version == add_version);
            generateReplicaTransferRequests(existed_replica_info,
                                            new_replica_info, transfer_tasks);
            if (transfer_tasks.empty()) {
                LOG(ERROR) << "No transfer tasks generated in replicate "
                              "operation, key: "
                           << key;
            ...
            while (!success && try_num < max_trynum_) {
                ...
                if (success) {
                    // Update status
                    updateReplicaStatus(transfer_tasks, status, key,
                                        add_version, new_replica_info);
                    break;
                }
                ++try_num;
            }
        }
        replica_allocator_.cleanUncompleteReplica(key, latest_version,
                                                  new_config.replica_num);
    } else if (new_config.replica_num < existed_replica_number) {
        // Need to remove replicas
        for (size_t i = 0; i < existed_replica_number - new_config.replica_num;
             ++i) {
            ReplicaInfo replica_info;
            replica_allocator_.removeOneReplica(key, replica_info,
                                                latest_version);
        }
    }
...
void DistributedObjectStore::checkAll() {
    ...
    std::unordered_map<ObjectKey, VersionList> &object_meta =
        replica_allocator_.getObjectMeta();
    ...
    for (auto &[key, version_list] : object_meta) {
        for (auto &[version, version_info] : version_list.versions) {
            ...
            uint32_t complete_replica_id =
                *version_info.complete_replicas.begin();
            ReplicaInfo complete_replica_info =
                version_info.replicas[complete_replica_id];

            for (auto &[replica_id, replica_info] : version_info.replicas) {
                if (replica_info.status == ReplicaStatus::PARTIAL) {
                    std::vector<TransferRequest> transfer_tasks;
                    generateReplicaTransferRequests(
                        complete_replica_info, replica_info, transfer_tasks);

                    bool success = false;
                    uint32_t try_num = 0;
                    while (!success && try_num < max_trynum_) {
                        std::vector<TransferStatusEnum> status;
                        success = transfer_agent_->doTransfers(transfer_tasks,
                                                               status);
                        if (success) {
                            updateReplicaStatus(transfer_tasks, status, key,
                                                version, replica_info);
                            break;
                        }
                        ++try_num;
                    }
                    if (!success) {
                        LOG(ERROR) << "Failed to recover partial replica "
                                   << replica_id << " for key " << key
                                   << ", version " << version;
                    }
                }
            }
...
TASK_STATUS DistributedObjectStore::getTaskStatus(TaskID task_id) {
    ...
    if (it == task_contexts_.end()) {
        ...
        return std::make_pair<OperationStatus, ERRNO>(OperationStatus::COMPLETE,
                                                      ERRNO::INVALID_KEY);
    }
...
// Private methods
TASK_STATUS DistributedObjectStore::updatePutStatus(
    TaskID task_id, std::vector<TransferStatusEnum> &status) {
    auto it = task_contexts_.find(task_id);
    if (it == task_contexts_.end()) {
        return std::make_pair<OperationStatus, ERRNO>(OperationStatus::COMPLETE,
                                                      ERRNO::INVALID_KEY);
    ...
    transfer_agent_->monitorTransferStatus(context->batch_id,
                                           context->task_size, status);

    // Check if all transfers are completed
    bool all_completed =
        std::all_of(status.begin(), status.end(), [](TransferStatusEnum s) {
            return s == TransferStatusEnum::COMPLETED ||
                   s == TransferStatusEnum::FAILED;
        });

    if (all_completed) {
        handlePutCompletion(context, status);
        // 判断在status中是否有TransferStatusEnum::FAILED,
        // 如果有一个则ERRNO为TRANSFER_FAIL 并返回
        auto op_status = std::make_pair<OperationStatus, ERRNO>(
    ...
    // 返回PENDING状态
    auto op_status = std::make_pair<OperationStatus, ERRNO>(
        OperationStatus::PENDING, ERRNO::OK);
    it->second->task_status = op_status;
    return op_status;
}

uint64_t DistributedObjectStore::calculateObjectSize(
...
void DistributedObjectStore::generateWriteTransferRequests(
    const ReplicaInfo &replica_info, const std::vector<Slice> &slices,
    std::vector<TransferRequest> &transfer_tasks) {
    size_t written = 0;
    ...
    for (const auto &handle : replica_info.handles) {
        ...
        while (shard_offset < handle->size && input_idx < slices.size()) {
            ...
            request.source =
                static_cast<char *>(current_slice.ptr) + input_offset;
            request.length = to_write;
            request.target_id = handle->segment_id;
            request.target_offset =
                reinterpret_cast<uint64_t>(handle->buffer) + shard_offset;
...
void DistributedObjectStore::generateReadTransferRequests(
    const ReplicaInfo &replica_info, size_t offset,
    const std::vector<Slice> &slices,
    std::vector<TransferRequest> &transfer_tasks) {
    size_t total_size = 0;
    for (const auto &slice : slices) {
    ...
    for (const auto &handle : replica_info.handles) {
        ...
        size_t shard_start =
            (remaining_offset > handle->size) ? 0 : remaining_offset;
        remaining_offset = (remaining_offset > handle->size)
                               ? remaining_offset - handle->size
                               : 0;
        ...
        while (shard_start < handle->size && bytes_read < total_size) {
            size_t bytes_to_read =
                std::min({handle->size - shard_start,
                          slices[output_index].size - output_offset,
                          total_size - bytes_read});

            request.source =
                static_cast<char *>(slices[output_index].ptr) + output_offset;
            request.target_id = handle->segment_id;
            request.target_offset =
                reinterpret_cast<uint64_t>(handle->buffer) + shard_start;
...
void DistributedObjectStore::generateReplicaTransferRequests(
    ...
    for (const auto &handle : existed_replica_info.handles) {
        slices.push_back(
            Slice{reinterpret_cast<void *>(handle->buffer), handle->size});
    }
...
bool DistributedObjectStore::validateTransferRequests(
    ...
    for (size_t task_id = 0; task_id < transfer_tasks.size(); ++task_id) {
        ...
        LOG(INFO) << "Segment id: " << task.target_id
                  ...
                  << ", task target offset: "
                  << reinterpret_cast<void *>(task.target_offset);

        // Find the corresponding BufHandle
        const auto &handle = replica_info.handles[handle_index];

        // Validate source address
        if (static_cast<char *>(slices[input_idx].ptr) + input_offset !=
            task.source) {
            LOG(ERROR) << "Invalid source address. Expected: "
                       << static_cast<char *>(slices[input_idx].ptr) +
                              input_offset
                       << ", Actual: " << task.source;
            return false;
        }
        ...
        uint64_t expected_target_offset =
            reinterpret_cast<uint64_t>(handle->buffer) + shard_offset;
        ...
        total_written_by_handle[reinterpret_cast<uint64_t>(handle->buffer)] +=
            task.length;
        ...
        if (input_offset == slices[input_idx].size ||
            task_id == transfer_tasks.size() - 1) {
    ...
    LOG(INFO) << "----------All transfer tasks validated "
                 "successfully.----------------";
    return true;
}
...
bool DistributedObjectStore::validateTransferReadRequests(
    ...
    for (const auto &request : transfer_tasks) {
        ...
        for (size_t i = 0; i < slices.size(); ++i) {
            if (request.source >= slices[i].ptr &&
                request.source <
                    static_cast<char *>(slices[i].ptr) + slices[i].size) {
                valid_source = true;
                break;
            }
        }
        ...
        for (const auto &handle : replica_info.handles) {
            if (request.target_id == handle->segment_id &&
                request.target_offset >=
                    reinterpret_cast<uint64_t>(handle->buffer) &&
                request.target_offset + request.length <=
                    reinterpret_cast<uint64_t>(handle->buffer) + handle->size) {
                valid_target = true;
                break;
            }
        }
    ...
    LOG(INFO) << "----------All transfer read tasks validated "
                 "successfully.----------------";
    return true;
}
